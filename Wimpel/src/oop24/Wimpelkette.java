...
public class Wimpelkette {
    ...
    // invocation
    private void generiereOptimaleKette(ArrayList<Wimpel> wimpelkette, ArrayList<Wimpel> result,
        ...
        if (wimpelkette.isEmpty()) {
            // wimpelkette.add(new
            // Wimpel(unsortedwimpel.get(getMaxCountElement()).getColor(), 1));
        }
        for (int i = 0; i < unsortedwimpel.size(); i++) {
            /*
             ...
             * unsortedwimpel.set(unsortedwimpel.get(i).getColor(),
             * unsortedwimpel.get(i).getCount() - 1);
             * }
             */
        }
     ...
     * if (this.unsortedwimpel.get(i) > 0) {
     * empty = false;
     * } else {
     * empty = true;
     * }
    ...
    // check if this still works and make it so that it takes a wimpelkette as
    // parameter
    // calculates the quality of the current wimpelkette
    // parameter is the wimpelkette to check
    public int[] getQuality(ArrayList<Wimpel> wimpelkette) {
        ...
        for (int i = 0; i < this.wimpelkette.size(); i++) {
            for (int j = 0; j < colorlist.size(); j++) {
                ...
                if (this.wimpelkette.get(i).getColor() == colorlist.get(j)) {
                    occurrence = true;
                    break;
                } else {
                    occurrence = false;
            ...
            if (occurrence == false) {
                colorlist.add(this.wimpelkette.get(i).getColor());
            }
        }
        ...
        for (int i = 0; i < colorlist.size(); i++) {
            ...
            for (int j = 0; j < this.wimpelkette.size(); j++) {
                if (this.wimpelkette.get(j).getColor() == colorlist.get(i)) {
                    for (int k = j + 1; k < this.wimpelkette.size(); k++) {
                        // vergleich j!=k nicht notwendig
                        if (this.wimpelkette.get(k).getColor() == colorlist.get(i)) {
